...
pub(crate) fn process_message(
    ...
    println!("process_message - starting message processing");
    ...
    for (instruction_index, ((program_id, instruction), program_indices)) in message
        ...
        .enumerate()
    {
        println!("process_message - processing instruction {} of {}", instruction_index + 1, message.num_instructions());
        ...
        println!("process_message - instruction fixup took: {:?}", fixup_start.elapsed());

        let accounts_setup_start = Instant::now();
        let mut instruction_accounts = Vec::with_capacity(instruction.accounts.len());
        for (instruction_account_index, index_in_transaction) in
            instruction.accounts.iter().enumerate()
        {
        ...
        println!("process_message - instruction accounts setup took: {:?}", accounts_setup_start.elapsed());
        ...
        let (result, process_instruction_us) = measure_us!({
            if let Some(precompile) = get_precompile(program_id, |feature_id| {
                ...
                println!("process_message - executing precompiled program");
                invoke_context.process_precompile(
                    precompile,
                    instruction.data,
                    &instruction_accounts,
                ...
                println!("process_message - executing regular program");
                invoke_context.process_instruction(
                    instruction.data,
                    &instruction_accounts,
                    program_indices,
        ...
        println!("process_message - instruction execution took: {:?}", execution_start.elapsed());
        ...
        *accumulated_consumed_units =
        ...
        execute_timings
            ...
            .process_instructions
            .total_us += process_instruction_us;
        println!("process_message - metrics update took: {:?}", metrics_update_start.elapsed());

        let error_handling_start = Instant::now();
        if let Err(err) = result {
            println!("process_message - instruction {} failed with error", instruction_index);
            println!("process_message - total processing time: {:?}", total_process_start.elapsed());
            return Err(TransactionError::InstructionError(instruction_index as u8, err));
        }
        println!("process_message - error handling took: {:?}", error_handling_start.elapsed());
    }
    println!("process_message - successfully processed all instructions");
    println!("process_message - total processing time: {:?}", total_process_start.elapsed());
